...
@limiter.limit("100/minute")
def get_bank_list(request: Request, db: Session = Depends(get_db)) -> Page[ProvicesOut]:
...
@router.get("/villages")
@limiter.limit("100/minute")
def get_bank_list(request: Request, db: Session = Depends(get_db)) -> Page[VillagesOut]:
